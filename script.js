...
import { initializeApp } from "[https://www.gstatic.com/firebasejs/11.9.0/firebase-app.js](https://www.gstatic.com/firebasejs/11.9.0/firebase-app.js)";
import { getAI, getGenerativeModel } from "[https://www.gstatic.com/firebasejs/11.9.0/firebase-ai.js](https://www.gstatic.com/firebasejs/11.9.0/firebase-ai.js)";
import { getAuth, GoogleAuthProvider, signInWithPopup, signOut, onAuthStateChanged } from "[https://www.gstatic.com/firebasejs/11.9.0/firebase-auth.js](https://www.gstatic.com/firebasejs/11.9.0/firebase-auth.js)";
import { getFirestore, collection, addDoc, query, where, getDocs, orderBy, serverTimestamp, doc, getDoc, setDoc } from "[https://www.gstatic.com/firebasejs/11.9.0/firebase-firestore.js](https://www.gstatic.com/firebasejs/11.9.0/firebase-firestore.js)";
...
/**
 * Extracts and parses a JSON object from a raw string, which may include markdown code blocks.
 * This function is robust and attempts to fix common JSON errors like trailing commas.
 * @param {string} rawText The raw text response from the AI.
 * @returns {object|null} The parsed JSON object, or null if parsing fails.
 */
function extractAndParseJson(rawText) {
    // 1. Find the JSON block. It might start with ```json and end with ```, or just ```.
    const regex = /```(json)?\s*([\s\S]*?)\s*```/;
    const match = rawText.match(regex);
    
    // If no markdown block is found, assume the whole text might be JSON.
    let jsonString = (match && match[2]) ? match[2] : rawText;

    // 2. Sanitize the string: remove control characters that can break parsing.
    jsonString = jsonString.replace(/[\u0000-\u001F\u007F-\u009F]/g, "");

    // 3. First attempt to parse.
    try {
        return JSON.parse(jsonString);
    } catch (error) {
        console.warn("Initial JSON parse failed. Attempting to fix...", { error: error.message, string: jsonString });

        // 4. If it fails, try to fix common issues, like trailing commas.
        const fixedJsonString = jsonString
            .replace(/,\s*([}\]])/g, "$1") // Remove trailing commas in objects and arrays
            .trim(); 

        try {
            // 5. Second attempt to parse the fixed string.
            return JSON.parse(fixedJsonString);
        } catch (finalError) {
            console.error("Final JSON Parse Error:", finalError.message, "Original String:", rawText, "Attempted Fix:", fixedJsonString);
            return null; // If it still fails, give up.
        }
    }
}
